...
public class AnalogFlick : MonoBehaviour {
    //How many sections the flick circle is divided into
    ...
    // Gesture detection parameters. Might need tweaking
    ...
    [SerializeField]
    int _lastBucket; //Bucket is the region on the circle
    ...
    private void Update() {
        if(inputManager.skill_trigger){
            ...
            Vector2 stick = new Vector2(inputManager.cameraInputX, inputManager.cameraInputY);
    ...
    bool CheckPattern() {
        ...
        if (_patternMatch.TryGetValue(_patternInProgress, out Combo combo)) {
            ...
            if (inputManager.skill_trigger) {
                skillManager.doSkill(combo.name);
            }
            //TODO Do same for shooter
            _patternInProgress = "";
            return true;
        }
